...
def qOH_vs_qH2O_plot(gtab: QTable, **kwargs) -> go.Scatter:
    ...
    tracename = f"parent τ: {gtab['parent_tau_d'][0]:6.0f}, fragment τ: {gtab['fragment_tau_T'][0]:6.0f}"

    return go.Scatter(x=xs, y=ys, name=tracename, **kwargs)


def qOH_vs_qH2O_plots_combined(table_file: pathlib.PurePath, data_table: QTable, **kwargs) -> None:
