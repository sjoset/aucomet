...
vm_parameter_sets = [
    GridVsAccuracyParameterSet(
        ...
        radial_gridsize_list=np.linspace(30, 150, num=13, endpoint=True, dtype=int),
        angular_gridsize_list=np.linspace(30, 100, num=8, endpoint=True, dtype=int),
        radial_substep_list=np.linspace(50, 100, num=4, endpoint=True, dtype=int),
    ...
    if input(f"Generate all missing data for {vmps.parameter_set_id}? [N/y] ") in [
        ...
        for row in dataset_table:
            if row["exists"]:  # type: ignore
                continue
            if input(f"Generate data?") in ["y", "Y"]:  # type: ignore
