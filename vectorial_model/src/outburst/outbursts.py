...
def outburst_radial_density_plots(
    vmc: pyv.VectorialModelConfig,
    vmr: pyv.VectorialModelResult,
    out_file,
    variation_type,
    varying_production_key,
    varying_production_value,
    max_voldens=None,
):
    # plt, fig, ax1, ax2
    plt, _, ax1, ax2 = pyv.vmplotter.radial_density_plots(
        vmc, vmr, r_units=u.km, voldens_units=1 / u.cm**3, show_plots=False
    )
    ax1.set_title(
        f"{variation_type}: {varying_production_key} = {varying_production_value:05.2f} ago"
    )
    ax2.set_title(
        f"{variation_type}: {varying_production_key} = {varying_production_value:05.2f} ago"
    )
...
def outburst_column_density_plots(
    vmc: pyv.VectorialModelConfig,
    vmr: pyv.VectorialModelResult,
    out_file,
    variation_type,
    ...
    # plt, fig, ax1, ax2
    plt, _, ax1, ax2 = pyv.vmplotter.column_density_plots(
        vmc, vmr, r_units=u.km, cd_units=1 / u.cm**2, show_plots=False
    )
    ax1.set_title(
        f"{variation_type} {varying_production_key} = {varying_production_value:05.2f} ago"
    )
...
def outburst_column_density_plot_3d(
    vmc: pyv.VectorialModelConfig,
    vmr: pyv.VectorialModelResult,
    x_min,
    x_max,
    ...
    # plt, fig, ax, surf
    plt, _, ax, _ = pyv.column_density_plot_3d(
        vmc,
        vmr,
        x_min,
        x_max,
        ...
        cdens_units,
        show_plots=False,
        vmin=min_coldens.to(cdens_units).value,
...
def outburst_fragment_sputter(
    fsc: pyv.FragmentSputterPolar,
    dist_units,
    sputter_units,
    within_r,
    out_file,
    ...
    # plt, fig, ax, surf
    plt, _, ax = pyv.plot_fragment_sputter(
        fsc,
        dist_units=dist_units,
        sputter_units=sputter_units,
        within_r=within_r,
        show_plots=False,
    )
    ax.set_title(
...
def main():
    ...
    vmc_set = pyv.vectorial_model_config_from_yaml(args.parameterfile[0])
    ...
    for index, vmc in enumerate(vmc_set):
        ...
        print(
            f"Calculating for {varying_production_key} = {varying_production_value:05.2f} :"
        )
    ...
    for frame_number, (vmc, vmr) in enumerate(list(zip(vmc_set, vmrs))):
        ...
        plotbasename = file_string_id_from_parameters(
            vmc,
        ...
        outburst_column_density_plot_3d(
            vmc,
            vmr,
            x_min=-100000 * u.km,
            x_max=100000 * u.km,
